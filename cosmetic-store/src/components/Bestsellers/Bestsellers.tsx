...
import BestellerItem from './BestellerItem/BestellerItem';
...
const Bestsellers = () => {
    ...
    const scrollCarousel = (direction : string) => {
        if (carousel.current) {
            carousel.current.scrollLeft += direction === 'right' ? img + 50  : -img - 50 
        }
    }
    ...
    useEffect(() => {
        ...
        const images = document.querySelectorAll('.besteller__item')
    ...
    return (
        <section className="bestellers__section">
            <div className="bestellers__title">Бестселлеры</div>
            <div className="bestellers__slider" ref={carousel}>
                <BestellerItem title='ТОНИЗИРУЮЩАЯ СЫВОРОТКА - БАЗА СО СВЕТООТРАЖАЮЩИМИ ЧАСТИЦАМИ' img='irc' desc='skin glow' price={2231} />
                <BestellerItem title='Сыворотка' img='irc' desc='7 acids serum SKIN RENEWAL' price={2345} />
                <BestellerItem title='МИСТ ДЛЯ ЛИЦА' img='irc' desc='4d extra hydrating mist' price={2231} />
                <BestellerItem title='ТЕПЛАЯ АЛЬГИНАТНАЯ МАСКА С ЧЕРНОЙ ИКРОЙ' img='irc' desc='relax spa' price={2231} />
                <BestellerItem title='ТОНИЗИРУЮЩАЯ СЫВОРОТКА - БАЗА СО СВЕТООТРАЖАЮЩИМИ ЧАСТИЦАМИ' img='irc' desc='skin glow' price={2231} />
                <BestellerItem title='ТЕПЛАЯ АЛЬГИНАТНАЯ МАСКА С ЧЕРНОЙ ИКРОЙ' img='irc' desc='relax spa' price={2231} />
                <BestellerItem title='МИСТ ДЛЯ ЛИЦА' img='irc' desc='4d extra hydrating mist' price={2231} />
                <BestellerItem title='ТОНИЗИРУЮЩАЯ СЫВОРОТКА - БАЗА СО СВЕТООТРАЖАЮЩИМИ ЧАСТИЦАМИ' img='irc' desc='skin glow' price={2231} />
            ...
            <div className="besteller__arrows">
