...
const Composition = () => {
    ...
    const scrollCarousel = (direction : string) => {
        if (carousel.current) {
            carousel.current.scrollLeft += direction === 'right' ? img : -img
        }
    }
    ...
    useEffect(() => {
        ...
        const images =  document.querySelectorAll<HTMLImageElement>('.composition__item')


        const handleImg = () => {
            setImg(images[0].clientWidth)
        }
    ...
    return (
        <section className="composition__section">
            <div className="composition__slider">
                <div className="composition__slider__container">
                    <div className="slider__arrow__left" onClick={() => {scrollCarousel('left')}}>
                    ...
                    <div className="composition__slider__carousel" ref={carousel}>
                        <CompositionItem img="4D" desc="4D гиалуроновая кислота"/>
                        <CompositionItem img="nag" desc="Hyafactor NAG "/>
                        <CompositionItem img="c" desc="Витамин C"/>
                        <CompositionItem img="ferulin" desc="Феруловая кислота"/>
                        <CompositionItem img="pha" desc="7 AHA BHA PHA кислот"/>
                        <CompositionItem img="prebiotik" desc="Адаптивный растительный пребиотик"/>
                        <CompositionItem img="peptidi" desc="Пептиды"/>
                        <CompositionItem img="retinol" desc="Ретинол"/>
                        <CompositionItem img="gaba" desc="GABA"/>
                        <CompositionItem img="aminokisloti" desc="Аминокислоты"/>
                        <CompositionItem img="4D" desc="4D гиалуроновая кислота"/>
                        <CompositionItem img="nag" desc="Hyafactor NAG "/>
                        
                    </div>
                    <div className="slider__arrow__right" onClick={() => {scrollCarousel('right')}}>
                        <svg width="24" height="14" viewBox="0 0 24 14" fill="none" xmlns="http://www.w3.org/2000/svg">
