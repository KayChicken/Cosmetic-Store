...
const SliderItems = () => {
    ...
    const [firstItem, setFirstImg] = useState<number>(0)
    


    const scrollScroller = (direction : string) => {
        if (carousel.current) {
            carousel.current.scrollLeft += direction === 'right' ? firstItem : -firstItem
        }
    ...
    useEffect(() => {
        ...
        const images = document.querySelectorAll<HTMLImageElement>('.slider__item')
        setFirstImg(images[0].clientWidth)
    ...
    return (
        <section className='slider__items'>
            <div className="slider__container">
                <div className="slider__arrow__left" onClick={() => scrollScroller('left')} >
                ...
                <div className="slider__content" ref={carousel}>
                    <SliderItem img='manyCosmetic' title="Диагностика" />
                    <SliderItem img='irc' title="Жирность" />
                    <SliderItem img='blackPoints' title="Черные точки" />
                    <SliderItem img='dryness' title="Сухость" />
                    <SliderItem img='antiage' title="Антиэдж" />
                    <SliderItem img='evenTone' title="Ровный тон" />
                    <SliderItem img='pigmetation' title="Пигментация" />
                    <SliderItem img='puffiness' title="Отечность" />
                    <SliderItem img='manyCosmetic' title="Диагностика" />
                    <SliderItem img='irc' title="Жирность" />
                    <SliderItem img='blackPoints' title="Черные точки" />
                </div>
                <div className="slider__arrow__right" onClick={() => scrollScroller('right')}>
                    <svg width="24" height="14" viewBox="0 0 24 14" fill="none" xmlns="http://www.w3.org/2000/svg">
